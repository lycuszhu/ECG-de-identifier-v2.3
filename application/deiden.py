...
def process_mindray_file_internal(file_path, output_path, csv_writer):
    tree = ET.parse(file_path)
    root = tree.getroot()
...
def process_philips_file_internal(file_path, output_path, csv_writer):
    namespace = {'ns': 'http://www3.medical.philips.com'}
    tree = ET.parse(file_path)
    root = tree.getroot()
...
def process_mortara_file_internal(file_path, output_path, csv_writer):
    tree = ET.parse(file_path)
    root = tree.getroot()
...
def process_mindray_file(file_path, output_path):
    csv_file_path = os.path.join(os.path.dirname(output_path), 'id_mappings.csv')
    ...
    with open(csv_file_path, mode='a', newline='') as csvfile:
        ...
        process_mindray_file_internal(file_path, output_path, csv_writer)
...
def process_philips_file(file_path, output_path):
    ...
    with open(csv_file_path, mode='a', newline='') as csvfile:
        ...
        if not file_exists or os.stat(csv_file_path).st_size == 0:
            csv_writer.writeheader()
        process_philips_file_internal(file_path, output_path, csv_writer)
...
def process_mortara_file(file_path, output_path):
    ...
    with open(csv_file_path, mode='a', newline='') as csvfile:
        ...
        if not file_exists or os.stat(csv_file_path).st_size == 0:
            csv_writer.writeheader()
        process_mortara_file_internal(file_path, output_path, csv_writer)
